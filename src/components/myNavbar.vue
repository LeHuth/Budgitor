<template>
  <header class="relative isolate z-10 bg-background-50">
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
      <div class="flex lg:flex-1">
        <a href="#" class="-m-1.5 p-1.5">
          <span class="sr-only">Your Company</span>
<!--          <img class="h-8 w-auto" src="https://tailwindui.com/img/logos/mark.svg?color=indigo&shade=600" alt="" />-->
          <BudgitorIcon class="h-8 w-auto fill-accent-500 stroke-accent-600" />
        </a>
      </div>
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-12">
        <Popover>
          ...
          <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-1">
            <PopoverPanel class="absolute inset-x-0 top-0 -z-10 bg-background-base pt-14 shadow-lg ring-1 ring-gray-900/5">
              <div class="mx-auto grid max-w-7xl grid-cols-4 gap-x-4 px-6 py-10 lg:px-8 xl:gap-x-8">
                <div v-for="item in products" :key="item.name" class="group relative rounded-lg p-6 text-sm leading-6 hover:bg-background-100">
                  <div class="flex h-11 w-11 items-center justify-center rounded-lg bg-gray-50 group-hover:bg-background-50">
        ...
        <a @click="logRows()" href="#" class="text-sm font-semibold leading-6 text-gray-900">Log Rows</a>
      </PopoverGroup>
    ...
    <Dialog as="div" class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
      ...
      <DialogPanel class="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-background-base px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
